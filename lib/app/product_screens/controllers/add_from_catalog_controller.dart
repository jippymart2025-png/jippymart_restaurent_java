...
class AddFromCatalogController extends GetxController {
  ...
  List<VendorCategoryModel> get filteredCategories {
    ...
    if (search.isEmpty) return categoryList.toList();
  ...
  final Rx<VendorCategoryModel?> selectedCategory = Rx<VendorCategoryModel?>(null);

  // ---------------------------------------------------------------------------
  // MASTER PRODUCTS
  // ---------------------------------------------------------------------------

  final RxList<MasterProductModel> masterProducts = <MasterProductModel>[].obs;
  ...
  Future<Map<String, int>> _loadOutletCategoryIdMap() async {
    ...
    try {
      final outletResult =
      await FireStoreUtils.getOutletDetailsWithProducts(outletId: outletId);
      ...
      for (final cat in outletResult.categories) {
        ...
        if (categoryId.isNotEmpty && outletCatId != null && outletCatId > 0) {
  ...
  Future<void> selectCategory(VendorCategoryModel? category) async {
    ...
    if (category?.id?.isNotEmpty != true) return;
  ...
  Future<void> _resolveOutletCategoryId(VendorCategoryModel category) async {
    try {
  ...
  Future<void> loadMasterProducts({bool append = false}) async {
    ...
    try {
      ...
      if (res != null) {
        ...
        print("Loaded Products Count => ${res.products.length}");
      }
    } catch (e) {
  ...
  SelectedProductModel _buildSelection(MasterProductModel product) {
    ...
    if (product.isExisting == true) {
      ...
      for (final t in product.vendorAvailableTimings ?? []) {
        if (t.day != null && t.timeslot != null) {
          final slots = t.timeslot!
              .map<TimeRangeItem>(
                (s) => TimeRangeItem(
              productAvailableTimingId: s.productAvailableTimingId ?? 0,
              from: s.from ?? '',
              to: s.to ?? '',
            ),
          )
      ...
      final options = (product.vendorOptions ?? [])
          .map(
            (o) => OptionItem(
          id: o.id ?? '',
          title: o.title ?? '',
          ...
          productVariantOptionsId: int.tryParse(o.id ?? '') ?? 0,
          productVariantGroupValuesId: 0,
          productVariantGroupsId: 0,
        ),
      )
          .toList();

      final merchant = double.tryParse(product.vendorMerchantPrice ?? '') ??
      ...
      return SelectedProductModel(
        ...
        categoryName: product.categoryName ?? selectedCategory.value?.title,
        imageLink: product.photo,
        isVeg: product.veg ?? false,
        hasProductVariants: options.isNotEmpty,
        vendorProductId: product.vendorProductId,
        merchantPrice: merchant,
        onlinePrice: double.tryParse(product.vendorPrice ?? '') ?? merchant,
        discountPrice: double.tryParse(product.vendorDisPrice ?? '') ?? 0,
  ...
  void updateMerchantPrice(String id, double value) {
    _update(id, (s) {
  ...
  void setOptions(String id, List<OptionItem> options) {
    _update(id, (s) {
      s.options = options;
      s.hasProductVariants = options.isNotEmpty;
    });
  }
  ...
  String? validateForSave() {
    ...
    for (final e in selectedProducts.entries) {
      ...
      if (product.discountPrice > product.onlinePrice) {
  ...
  // ---------------------------------------------------------------------------
  // DAY NAME → DAY OF WEEK ID
  // ---------------------------------------------------------------------------

  int _dayToId(String day) {
    const map = {
      'monday': 1,
      'tuesday': 2,
      'wednesday': 3,
  ...
  // ---------------------------------------------------------------------------
  // BUILD JAVA API REQUEST
  // ---------------------------------------------------------------------------

  AddProductsFromMasterRequest buildMasterRequest({
    required int outletId,
    required int categoryId,
  }) {
    final catName = selectedCategory.value?.title ?? '';

    final items = selectedProducts.values.map((sel) {
      // -------- TIMINGS --------
      final timings = <ProductTimingRequest>[];
      sel.availableTimings.forEach((day, slots) {
        for (final slot in slots) {
          timings.add(
            ProductTimingRequest(
              productAvailableTimingId: slot.productAvailableTimingId,
              dayOfWeekId: _dayToId(day),
              startTime: slot.from,
              endTime: slot.to,
            ),
          );
      ...
      // -------- VARIANT GROUPS --------
      final variantGroups = <VariantGroupRequest>[];
      if (sel.options.isNotEmpty) {
        final groupId = sel.options.first.productVariantGroupsId;
        ...
        variantGroups.add(
          VariantGroupRequest(
            ...
            options: sel.options
                .map(
                  (opt) => VariantOptionRequest(
                productVariantOptionsId: opt.productVariantOptionsId,
                productVariantGroupValuesId:
                opt.productVariantGroupValuesId,
                priceType: 'FIXED',
                variantPrice: double.tryParse(opt.price) ?? 0,
              ),
            )
                .toList(),
          ),
      ...
      // ✅ Resolve per-product category with fallbacks
      final resolvedCategoryId =
          int.tryParse(sel.categoryId ?? '') ?? categoryId;

      final resolvedCategoryName =
      (sel.categoryName != null && sel.categoryName!.isNotEmpty)
          ? sel.categoryName!
          : catName;

      return AddProductFromMasterItem(
        masterProductId: int.tryParse(sel.masterProductId) ?? 0,
    ...
    return AddProductsFromMasterRequest(
      outletId: outletId,
  ...
  Future<bool> saveAndCaptureResponse() async {
    ...
    if (err != null) {
      ...
      lastStoreResponse = AddProductsFromMasterResponse();
      return false;
    }
    ...
    try {
      ...
      final categoryId = resolveOutletCategoryIdForSave();
      if (categoryId <= 0) {
        print("ERROR: Invalid categoryId = $categoryId");
        lastStoreResponse = AddProductsFromMasterResponse();
        return false;
      }
      ...
      final outletId = FireStoreUtils.resolveActiveOutletId();
      if (outletId <= 0) {
        print("ERROR: Invalid outletId = $outletId");
        lastStoreResponse = AddProductsFromMasterResponse();
        return false;
      }

      final selected = selectedProducts.values.toList();

      print("====================================");
      print("ADD PRODUCTS FROM MASTER");
      print("Outlet ID: $outletId");
      print("Category ID: $categoryId");
      print("Selected Products: ${selected.length}");
      print("====================================");

      // 4) Build request
      final request = buildMasterRequest(
        outletId: outletId,
        categoryId: categoryId,
      );

      // 5) Call Java API
      final res = await FoodApiService.addProductsToOutletFromMaster(request);

      lastStoreResponse = res;

      print("====================================");
      print("JAVA API RESPONSE");
      print("Saved Count: ${res.savedCount}");
      print("Skipped Count: ${res.skippedCount}");
      print("Saved Names: ${res.savedNames}");
      print("Skipped Names: ${res.skippedNames}");
      print("====================================");

      // 6) Refresh inventory
      if (res.savedCount > 0) {
        await _refreshInventoryAfterSave(selected);
      }

      return res.savedCount > 0;
    } catch (e, st) {
      print("AddFromCatalogController.saveAndCaptureResponse error: $e");
      print(st);
